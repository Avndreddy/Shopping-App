...
function OpenOverlay() {
    ...
    const listItems=['Men','Women','Kids','Home & Living'];
    const listItem = [
    ...
    const handleClickedOutside = (event) => {
        if (outsideClicked.current && !outsideClicked.current.contains(event.target)) {
    ...
    return (
        isOverlayOpened && (<div ref={outsideClicked} className="overlay" onClick={(e) => e.stopPropagation()}>
                {listItem.map((value,index)=>
                <div key={index}>
                    <div onClick={()=>openList(index)} className="Overlay-ListItems" key={index}>
                    ...
                    </div>
                    {
                        selectedIndex === index &&(
                            <div>
                                {
                                    value.subItems.map((subItem,subindex)=><div className="Overlay-Sub-ListItems" key={subindex}>{subItem}</div>)
                                }
                            </div>
                        )
                    }
